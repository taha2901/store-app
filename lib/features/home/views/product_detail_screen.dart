...
class _ProductDetailScreenState extends State<ProductDetailScreen> {
  ...
  // ✅ Add to SQLite then navigate to CartScreen
  Future<void> _goToCart() async {
    ...
    Navigator.push(
      ...
      // Reset the button when coming back from cart
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Icon(
                            ...
                            color: isFav ? AppColors.heartColor : AppColors.hintText,
                            size: 18,
                          ),
                        ),
                      ),
                ...
                child: Column(
                  ...
                  children: [
                          ...
                          child: Column(
                            ...
                            children: [
                              Container(
                                ...
                                child: Text(_formatCategory(p.category),
                              ...
                              Text(p.title,
                                  style: const TextStyle(fontSize: 22,
                                      fontWeight: FontWeight.w700, color: AppColors.title, letterSpacing: -0.4)),
                              if (p.brand != null) ...[
                            ...
                            children: [
                              GestureDetector(
                                ...
                                child: const Padding(
                                  ...
                                  child: Icon(Icons.remove, size: 16, color: AppColors.title),
                                ),
                              ),
                              Text('$quantity',
                              ...
                              GestureDetector(
                                ...
                                child: const Padding(
                                  padding: EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                                  child: Icon(Icons.add, size: 16, color: AppColors.title),
                                ),
                              ),
                            ],
                          ),
                          ...
                          Container(
                            ...
                            child: Text('-${p.discountPercentage.toStringAsFixed(0)}% OFF',
                                style: const TextStyle(fontSize: 10,
                                    fontWeight: FontWeight.w700, color: AppColors.heartColor)),
                          ),
                        ],
                      ],
                    ),
                    ...
                    Wrap(
                      ...
                      children: [
                        InfoBadge(icon: Icons.inventory_2_outlined, label: '${p.stock} in stock'),
                        InfoBadge(icon: Icons.local_shipping_outlined, label: p.shippingInformation),
                        InfoBadge(icon: Icons.assignment_return_outlined, label: p.returnPolicy),
                        if (p.warrantyInformation.isNotEmpty)
                          InfoBadge(icon: Icons.verified_outlined, label: p.warrantyInformation),
                      ],
                    ),
                    ...
                    Text(p.description,
                        style: const TextStyle(fontSize: 14, color: AppColors.bodyText, height: 1.6)),
                    ...
                    if (p.tags.isNotEmpty) ...[
                      Wrap(
                        ...
                        children: p.tags.map((tag) => Container(
                          ...
                          child: Text('#$tag', style: const TextStyle(fontSize: 11, color: AppColors.bodyText)),
                        )).toList(),
                      ),
                      const SizedBox(height: 16),
                    ],
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          Icon(
                            p.isInStock ? Icons.check_circle_outline : Icons.warning_amber_outlined,
                            size: 18,
                            color: p.isInStock ? const Color(0xFF4CAF50) : Colors.orange,
                          ),
                          const SizedBox(width: 8),
                          Text(p.availabilityStatus,
                              style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
                                  color: p.isInStock ? const Color(0xFF4CAF50) : Colors.orange)),
                          const Spacer(),
                          Text('Min order: ${p.minimumOrderQuantity}',
                    ...
                    // ══════════════════════════════════════════
                    // ✅ Add to Cart → SQLite → CartScreen
                    // ══════════════════════════════════════════
                    GestureDetector(
                      onTap: _addedToCart ? _goToCart : _goToCart,
                      child: AnimatedContainer(
                    ...
                    if (p.reviews.isNotEmpty) ...[
                      ...
                      ...p.reviews.take(2).map((review) => Padding(
                            padding: const EdgeInsets.only(bottom: 10),
                            child: CommentTile(
                              name: review.reviewerName,
                              comment: review.comment,
                              rating: review.rating,
                              timeAgo: _formatDate(review.date),
                            ),
                          )),
                    ],
                  ],
